...
const pathsPresent = (session) => new Set((session.turns ?? []).flatMap((t) => t.files ?? []).map((f) => safePath(f.path))).size;
/**
 ...
 */
export const pathLimit = (session) => {
    const shown = pathsPresent(session);
    if (session.recordedPaths === true)
        return ('FILE PATHS ARE RECORDED. Each turn below carries the files named by the file-path ' +
    ...
    if (session.recordedPaths === false)
        return ('FILE PATHS WERE NOT RECORDED. This reading was extracted with --no-paths, so the ' +
    ...
    if (shown > 0)
        return (`WHETHER FILE PATHS WERE RECORDED IS UNKNOWN, BUT ${shown} DISTINCT PATH(S) ARE HERE. This reading ` +
    ...
    return ('WHETHER FILE PATHS WERE RECORDED IS UNKNOWN. This reading predates the field that ' +
...
const safeFiles = (files) => (files ?? []).map((f) => ({ path: safePath(f.path), count: f.count }));
// ---------------------------------------------------------------- text shapes
...
function artifactsCsv(session) {
    ...
    for (const t of session.turns)
        for (const f of safeFiles(t.files))
            files[f.path] = (files[f.path] ?? 0) + f.count;
    for (const [path, count] of Object.entries(files).sort((x, y) => y[1] - x[1] || (x[0] < y[0] ? -1 : x[0] > y[0] ? 1 : 0)))
        rows.push([cell('file'), cell(path), num(count), 'session']);
    rows.push([
        cell('file_touch'),
        cell(session.recordedPaths === true
 ...
 */
function filesTouchedRow(session) {
    ...
    if (session.recordedPaths === false)
        return `${n} (count only — extracted with --no-paths)`;
    if (distinct > 0)
...
export function buildBundle(session, intent, meta = {}) {
    ...
    // constant.
    const limits = [redactionLimit(session), pathLimit(session), ...LIMITS];
    const body = [
