 ...
 */
function schemaFailures(l) {
    const fams = (l.families || []).filter((f) => f.structured >= 25);
    if (fams.length < 2)
        return [];
    const rate = (f) => f.structuredFail / f.structured;
    ...
    return fams
        ...
        brief: `${f.structuredFail} of ${f.structured} structured runs in the \`${f.family}\` family did not return ` +
...
function rituals(items) {
    return items
        ...
        brief: `Typed ${i.count} times across ${i.sessions} sessions${i.repos?.length ? ` in ${i.repos.join(', ')}` : ''}.\n` +
...
if (isMain) {
    ...
    try {
        ...
        if (argv.includes('--close')) {
            ...
            const open = (await api(cfg, '/v1/tasks'));
        ...
        for (const c of found)
            byDetector.set(c.detector, [...(byDetector.get(c.detector) || []), c]);
