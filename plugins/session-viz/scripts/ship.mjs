...
export async function harvest({ minCount = 2 } = {}) {
    ...
    for (const f of listSessions()) {
        ...
        for (const t of s.turns) {
            ...
            if (key.length < 6 || SKIP.test(key) || MACHINE.test(key))
                continue;
            if (!groups.has(key))
                groups.set(key, { key, text: t.text.trim(), hits: [] });
    ...
    for (const g of groups.values()) {
        ...
        const tools = new Map();
        for (const h of worked)
            for (const n of h.toolNames)
...
const yamlString = (s) => `"${s.replace(/\p{Cc}+/gu, ' ').replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"`;
...
if (isMain) {
    ...
    if (target) {
        ...
        if (!item) {
            console.error(`no repeated prompt matching "${target}"`);
            process.exit(1);
        }
        if (!item.promote)
            console.error(`note: this one is classified "${item.kind}", not a ritual — writing anyway`);
        const f = commandFile(item);
        mkdirSync('.claude/commands', { recursive: true });
        if (existsSync(f.path)) {
            console.error(`${f.path} already exists — not overwriting`);
            process.exit(1);
        }
    ...
    if (misses.length)
        console.log('  Re-sending these reproduced the no-op. Rewrite them; a command would not help.');
