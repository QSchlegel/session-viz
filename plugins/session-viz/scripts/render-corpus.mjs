...
function chart(timeline) {
    if (timeline.length < 2)
        return '<div class="empty">Not enough weeks to plot a trend.</div>';
    const W = 1040;
    const H = 210;
    ...
    const y = (v) => pad.t + ih - (v / maxRate) * ih;
    ...
    const line = (key, color) => {
        ...
        const dots = timeline
            .map((b, i) => `<circle cx="${x(i).toFixed(1)}" cy="${y(b[key]).toFixed(1)}" r="3.2" fill="${color}"><title>${esc(b.week)}: ${pct(b[key])} ${key.replace('Rate', '')} (${b.turns} turns)</title></circle>`)
            .join('');
...
function heroSection(m) {
    const t = m.trend;
    if (!t.measurable)
        return `<div class="card"><p class="dim">Trend not measurable — ${esc(t.why)}.</p></div>`;
...
function taxonomySection(m) {
    const rows = Object.entries(m.taxonomy)
        .filter(([, v]) => v.count)
        .sort((a, b) => b[1].count - a[1].count);
    if (!rows.length)
        return '<div class="empty">No friction recorded anywhere in the corpus.</div>';
...
function signalsSection(m) {
    ...
    const cards = m.signals
        ...
        const strata = s.strata
            .map((st) => `<span class="stratum${st.counts ? '' : ' off'}">${esc(st.label)} · <b>${pct(st.on.rework)}</b> <span class="dim">(n=${st.on.n})</span> vs ${pct(st.off.rework)} <span class="dim">(n=${st.off.n})</span></span>`)
            .join('');
...
// ---------------------------------------------------------------- drill-downs
const cell = (label, value, hot) => `<div class="mcell${hot ? ' hot' : ''}"><div class="n">${esc(value)}</div><div class="l">${esc(label)}</div></div>`;
const mgrid = (cells) => `<div class="mgrid">${cells.filter(Boolean).join('')}</div>`;
const modelChips = (models) => Object.entries(models || {})
    .sort((a, b) => b[1] - a[1])
...
// the session view.
function incidentList(incidents, cap = 8) {
    if (!incidents.length)
        return '<p class="dim" style="font-size:13px;margin:0">No rework incidents.</p>';
  ...
  <div class="inner">
    ...
    ${p.worktrees.length > 1
            ? `<div class="lbl2">Worktrees (${p.worktrees.length})</div><div class="chips">${p.worktrees
                .map((w) => `<span class="chip2">${esc(w.name)} <b>${w.turns}</b>t${w.frictionTurns ? ` · ${w.frictionTurns}f` : ''}</span>`)
                .join('')}</div>`
            : ''}
...
<div class="note">The clearest evidence in the corpus: the same prompt sent twice means the first attempt produced nothing usable. Worth reading as a set — what they have in common is what to change.</div>`);
  ...
  <div class="body">
    ...
    if (e.corrections.length) {
        ...
        out.push(e.corrections
            .map((c) => `<details class="pair">
...
const KIND_LABEL = { package: 'package', tool: 'CLI tool', stack: 'stack file', skill: 'skill', mcp: 'MCP server' };
function graphSection(m) {
    const g = m.graph;
    if (!g || !g.nodes.length)
...
function modelsSection(m) {
    ...
    const adoption = m.timeline
        ...
        const segs = Object.entries(b.models)
            ...
            .map(([name, n]) => `<span style="width:${((n / total) * 100).toFixed(1)}%;background:${colorFor(name, names)}" title="${esc(b.week)} · ${esc(name)}: ${n} turns"></span>`)
            .join('');
...
// ---------------------------------------------------------------- render
export function render(m, advice) {
...
<style>${css()}${brandCss()}</style></head><body><div class="wrap">

${brandHeader({ command: '/qtrends' })}
...
${brandFooter({
        command: '/qtrends',
        // Every fragment this footer has ever printed, in the order it printed them.
        // These are the lines that make the numbers above readable by someone who was
        // not here when they were taken — how much transcript, how many sessions, how
        // much was skipped, and that nothing verbatim went in unredacted.
        facts: [
            `${(m.meta.transcriptBytes / 1048576).toFixed(0)} MB of transcript across ${m.meta.sessionCount} sessions`,
            `${m.meta.subagents.files} subagent transcripts not parsed`,
