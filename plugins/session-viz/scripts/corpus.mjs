...
function buildCaveats(model) {
    ...
    if (noModel)
        c.push(`${noModel.turns} turns had no model record because the request was aborted before anything ran. They are excluded from every per-model rate; folding them in would blame whichever model happened to be selected.`);
...
export async function buildCorpus({ projectFilter = null, since = null, limit = null } = {}) {
    ...
    const turns = sessions.flatMap((s) => s.turns.map((t) => ({ ...t, _session: s.sessionId, _project: repoName(s.cwd) || s.project })));
