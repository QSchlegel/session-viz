 ...
 */
export function brandMark(size = MIN_MARK) {
    const rects = [];
    for (let i = 0; i < CELLS; i++) {
        const kind = i === HOLLOW_AT ? 'sv-hollow' : i === ACCENT_AT ? 'sv-accent' : 'sv-settled';
        rects.push(`<rect class="sv-cell ${kind}" x="${ORIGIN + (i % 3) * PITCH}" y="${ORIGIN + Math.floor(i / 3) * PITCH}" ` +
 ...
 */
export function brandLockup(size, href) {
...
export function brandFooter(o) {
    ...
    for (const f of o.facts) {
        if (!f)
            continue;
        const fact = typeof f === 'string' ? { value: f } : f;
        const value = String(fact.value ?? '');
        if (!value)
            continue;
        parts.push((fact.label ? esc(fact.label) + ' ' : '') +
