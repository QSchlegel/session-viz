...
export async function shipReport(args) {
    ...
    if (state.url && state.url !== dest.url)
        return fail(`you turned shipping on for ${state.url}, but this run resolves to ${dest.url}`, ['  Nothing was sent. Turn it on again if the new destination is the one you want.']);
    ...
    const cred = { source: process.env.SESSION_VIZ_TOKEN ? 'env' : 'file', fingerprint: credentialFingerprint(dest.token) };
    if (state.credential && state.credential.fingerprint !== cred.fingerprint)
        return fail('the credential changed since you turned shipping on, so this run would send to a workspace you have not agreed to', [`  Consent was recorded for a ${state.credential.source} credential; this run uses a ${cred.source} one.`,
