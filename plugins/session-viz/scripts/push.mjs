...
async function sendSidecars(spine, dest, args, lines, out) {
    ...
    if (args.intentPath) {
        try {
            ...
            intentNote = `  INTENTS NOT SENT — could not read ${args.intentPath}: ${e.message}`;
        }
    }
    ...
    const withheldTitles = intentDoc ? projectIntent(intentDoc, typedPrompts(spine.turns)).withheld : 0;
