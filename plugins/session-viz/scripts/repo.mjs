...
export const repoRoot = (cwd) => {
    const s = String(cwd || '');
    if (CURSOR_WORKTREE.test(s))
        return cursorWorktreeRepo(s) ?? s.split(WORKTREE_SEP)[0];
 ...
 */
function cursorWorktreeRepo(cwd) {
...
export const repoName = (cwd) => repoRoot(cwd).replace(/^.*\//, '');
/**
 ...
 */
export const repoFromSlug = (slug) => slug.replace(/^-Users-[^-]+-/, '').split('--claude-worktrees-')[0];
