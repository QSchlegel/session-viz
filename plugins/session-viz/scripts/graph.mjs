...
const statusOf = (v) => INTENT_STATUSES.includes(String(v)) ? String(v) : 'unstated';
export function mergeAuthored(derived, graph, turnCount, extra) {
    ...
    if (!graph && !threads.length && !priors.length)
    ...
    // survives.
    const intentIds = new Map(); // slug -> namespaced id
    let badIntent = 0;
    let uncitedTurn = 0;
    {
        const hubs = new Map();
        const hubCount = new Map();
        const hubBirth = new Map();
        for (const t of threads.slice(0, MAX_INTENTS)) {
            ...
            if (typeof firstTurn === 'number') {
                ...
                if (cur === undefined || firstTurn < cur)
                    hubBirth.set(status, firstTurn);
            }
    ...
    for (const p of priors.slice(0, MAX_PRIOR)) {
        ...
        const ago = Number.isInteger(p?.sessionsAgo) && p.sessionsAgo > 0 ? p.sessionsAgo : null;
        ...
        // hub they hang off.
        const from = { session: sid, sessionsAgo: ago, daysAgo: days };
        ...
        nodes.push({
            ...
            // happened in it.
            note: `Conclusions the intent store kept from session ${sid.slice(0, 8)}` +
        ...
        for (const t of p?.intents || []) {
            ...
            nodes.push({
                ...
                note: (t?.summary ? `${String(t.summary).slice(0, 320)} ` : '') +
                    (cited.length
        ...
        for (const c of p?.graph?.concepts || []) {
            ...
            nodes.push({
                ...
                layer: 'authored',
                carried: from,
                note: (c.note ? `${String(c.note).slice(0, 320)} ` : '') +
                    (cited.length
                        ? `Cited turn ${cited.join(', ')} of session ${sid.slice(0, 8)}, not of this one.`
                        : 'No turns were cited for it.'),
                firstTurn: null,
        ...
        if (offered > MAX_PRIOR_NODES)
            carriedBudget += offered - MAX_PRIOR_NODES;
...
const WHY_UNMATCHED = {
    ...
    'maybe-node-builtin': 'the name is a node built-in that no installed package shadows, so nothing here can tell which was meant',
    'bom-incomplete': 'the bill is not a full closure, so absence from it is not evidence of being missing',
 ...
 */
export function mergeDependencies(base, bom, matched, source) {
    ...
    for (const m of matched?.matches ?? []) {
        ...
        if (m.entryIds.length > 1)
            ambiguous.push(m.root);
        for (const id of m.entryIds) {
            ...
            if (!have.has(depId)) {
                ...
                add(depId, 'dep', `${e.name}${e.version ? ` ${e.version}` : ''}`, {
                    note: `Declared by this repository, not observed in this session. ` +
    ...
    for (const t of source.turnFiles)
        for (const path of t.paths) {
            ...
            if (prev === undefined || t.index < prev)
                touchedAt.set(man.path, t.index);
        }
    let declaredDrawn = 0;
    for (const [path, turn] of [...touchedAt].sort((a, b) => a[0].localeCompare(b[0]))) {
        ...
        add(manId, 'manifest', path, {
            note: `A manifest in this repository, edited during this session. It declares ` +
        ...
        for (const id of directOf.get(man.id) ?? []) {
            ...
            if (!have.has(depId)) {
                ...
                declaredDrawn++;
                add(depId, 'dep', `${e.name}${e.version ? ` ${e.version}` : ''}`, {
                    note: `Declared by ${path}, which a turn of this session edited. Not observed ` +
    ...
    if (rest > 0)
        dropped.push({
            ...
            why: 'nothing measured in this session points at ' + many(rest, 'it', 'them') +
    ...
    for (const [why, n] of [...unmatched].sort((a, b) => b[1] - a[1]))
        dropped.push({
            ...
            why: WHY_UNMATCHED[why] ?? `the reading reported them as ${why}`,
        });
    if (ambiguous.length)
        dropped.push({
