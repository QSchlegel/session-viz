 ...
 */
export const band = (v) => (v && /^\d+\.\d+\.\d+$/.test(v) ? v.replace(/\d$/, 'x') : '0.0.x');
/** log2 buckets, capped. Also ported verbatim, and for the same reason. */
export const bucket = (n, cap) => Math.min(cap, Math.round(Math.log2(1 + Math.max(0, n))));
/**
 ...
 */
export const contribSlug = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9_-]+/g, '-')
...
export function validateFinding(f) {
    ...
    for (const [k, allowed] of Object.entries(ENUMS)) {
        if (!allowed.includes(String(o[k])))
            return `invalid ${k}`;
    }
    if (!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(String(o['task_class'])))
        return 'invalid task_class';
    if (!/^\d+\.\d+\.\d*x$/.test(String(o['cli_band'])))
        return 'invalid cli_band';
    if (!/^\d{4}-W\d{2}$/.test(String(o['iso_week'])))
        return 'invalid iso_week';
    const cost = o['cost_bucket'], tool = o['tool_bucket'];
    if (!Number.isInteger(cost) || cost < 0 || cost > 40)
        return 'invalid cost_bucket';
    if (!Number.isInteger(tool) || tool < 0 || tool > 14)
        return 'invalid tool_bucket';
    return null;
}
// ---------------------------------------------------------------- disclosure
/**
 ...
 */
export function describe(findings, runs, home) {
    ...
    return {
        bytes: body.length,
        fields: [...present].sort(),
        unknown: [...present].filter((k) => !FIELDS.includes(k)).sort(),
