        ...
        for (const e of ents) {
            if (e.isFile() || e.isSymbolicLink()) {
                if (wanted.has(e.name))
                    found.push({ dir, rel: relOf(root, dir), file: e.name });
            }
        }
        if (depth >= maxDepth) {
            for (const e of ents) {
...
function resolveFrom(packages, fromKey, name) {
    ...
    while (prefix !== null) {
        ...
        if (Object.prototype.hasOwnProperty.call(packages, cand))
            return cand;
        if (prefix === '')
            break;
 ...
 * flattering claim.
 */
export function readSbom(root, options = {}) {
    ...
    const readLockAt = (dirRel) => {
        if (!byDir.get(dirRel)?.has('package-lock.json'))
            return null;
        ...
        if (!parsed.ok) {
            ...
            out = { path, base: dirRel, packages: parsed.value.packages };
        }
        lockCache.set(dirRel, out);
        return out;
    };
    /** The nearest lockfile at or above a manifest's directory. */
    const nearestLock = (dirRel) => {
        let d = dirRel;
        while (d !== null) {
            const got = readLockAt(d);
            if (got && 'packages' in got)
                return { lock: got, status: 'read', reason: null, altPath: null };
            if (got)
                return { lock: null, status: 'unread', reason: got.unreadable, altPath: d ? `${d}/package-lock.json` : 'package-lock.json' };
    ...
    const declaredByManifest = new Map();
    for (const mf of manifestFiles) {
        ...
        for (const field of DEP_FIELDS) {
            ...
            for (const [name, range] of Object.entries(map))
                declared.push({ name, range: String(range), field });
        }
    ...
    for (const m of manifests) {
        ...
        if (workspaceMembers.has(dir))
            m.workspaceMember = true;
    }
    // ----------------------------------------------------------------- entries
    ...
    const idOfKey = (lock, key) => `npm:${lock.base ? `${lock.base}/` : ''}${key}`;
    const lockEntry = (lock, key, pkg) => {
    ...
    for (const m of manifests)
        if (m.workspaceMember && m.name)
            workspaceByName.set(m.name, m);
    /**
    ...
    const declareConstraint = (m, d) => {
        ...
        if (!ws && FIRST_PARTY_PROTOCOL.test(d.range)) {
            ...
            notes.push(`${d.name} is declared in ${m.path} as ${d.range}, a first-party protocol, ` +
    ...
    for (const lock of [...lockCache.values()].filter((l) => l !== null && 'packages' in l)) {
        if (!locksRead.has(lock.path))
            continue;
        for (const [key, pkg] of Object.entries(lock.packages)) {
            if (!key.includes('node_modules/'))
                continue; // '' and workspace members are manifests
            if (pkg.link === true)
                continue;
            lockEntry(lock, key, pkg);
        }
        for (const [key, pkg] of Object.entries(lock.packages)) {
            if (!key.includes('node_modules/') || pkg.link === true)
                continue;
            const fromId = idOfKey(lock, key);
            if (!entries.has(fromId))
                continue;
            for (const field of ['dependencies', 'optionalDependencies', 'peerDependencies']) {
                ...
                for (const [name, range] of Object.entries(map)) {
                    ...
                    if (tp.link === true && typeof tp.resolved === 'string') {
                        ...
                        if (tm) {
                            edges.push({ from: fromId, to: tm.id, kind: 'depends', field, constraint: String(range) });
                            continue;
                        }
                    }
    ...
    const closure = closureFrom.every((x) => x)
        ? 'from-lockfiles'
    ...
    const read = [{ ecosystem: 'npm', manifests: manifests.length, locksRead: locksRead.size }];
    return {
        root,
        status: 'ok',
        reason: null,
...
export function packageNameOf(spec) {
    ...
    if (s === '')
        return { name: null, reason: 'empty-specifier' };
    if (s.startsWith('.'))
        return { name: null, reason: 'relative-specifier' };
    if (s.startsWith('/'))
        return { name: null, reason: 'absolute-specifier' };
    if (s.startsWith('#'))
        return { name: null, reason: 'subpath-import' };
    if (s.startsWith('node:'))
        return { name: null, reason: 'node-builtin' };
    if (/^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(s))
        return { name: null, reason: 'url-specifier' };
    const parts = s.split('/');
    ...
    if (BUILTINS.has(name))
        return { name, reason: 'maybe-node-builtin' };
 ...
 * perfect hit rate.
 */
export function matchImportRoots(sbom, roots) {
...
export function graphOf(sbom) {
    ...
    for (const e of sbom.edges) {
        if (known.has(e.from) && known.has(e.to))
            ...
            dangling.push({ from: e.from, to: e.to, kind: String(e.kind) });
    }
...
const defaultGit = (root) => (args) => {
    ...
    if (r.error)
        return { ok: false, reason: r.error instanceof Error ? r.error.message : String(r.error) };
    if (r.status !== 0)
        return { ok: false, reason: firstLine(String(r.stderr ?? '')) || `git exited with status ${r.status}` };
 ...
 */
export function classifyVersionText(text) {
...
function ecosystemOf(pkg) {
    ...
    if (typeof id === 'string') {
        ...
        if (m)
            return { ecosystem: PURL_TYPE_TO_ECOSYSTEM[m[1]] ?? m[1], source: 'spdxid' };
    }
 ...
 */
export function readGithubSbomDocument(body) {
    ...
    if (!docRaw || typeof docRaw !== 'object')
        return { ok: false, reason: 'the response had no "sbom" document object' };
    const doc = docRaw;
    ...
    const packages = [];
    let ref = { describes: 'github-dependency-graph', ref: null, refSource: 'unknown', repository: null };
    for (const p of packagesRaw) {
        ...
        if (isSelf) {
            ...
            const purl = (() => {
                const refsArr = Array.isArray(pk['externalRefs']) ? pk['externalRefs'] : [];
                for (const r of refsArr) {
                    if (!r || typeof r !== 'object')
                        continue;
                    const loc = r['referenceLocator'];
                    if (typeof loc === 'string' && loc.startsWith('pkg:github/'))
                        return loc;
                }
                return null;
            })();
 ...
 */
export const ghSbomFetcher = async (repo) => {
    ...
    if (probe.kind !== 'fetch-failed')
        return { ok: false, status: probe.kind, reason: probe.reason };
    return {
        ok: false,
...
const EFFECT_NO_COMPARISON = 'no second reading was obtained, so nothing here says the two bills of materials agree; the local reading stands alone';
function noComparison(status, reason, sbom, refs, repository, notes) {
    return {
        status,
        reason,
 ...
 */
function compareVersions(localVersions, githubVersions) {
    if (localVersions.length === 0 || githubVersions.length === 0)
    ...
    if (lockedLocal.length !== localVersions.length || lockedGithub.length !== githubVersions.length)
        return 'not-comparable-range';
    const l = new Set(lockedLocal.map((v) => v.value));
    const g = new Set(lockedGithub.map((v) => v.value));
    if (l.size === g.size && [...l].every((v) => g.has(v)))
        return 'same';
    return 'differs';
}
...
function compareLicenses(localLicenses, githubLicenses) {
    if (localLicenses.length === 0 && githubLicenses.length === 0)
        return 'both-unknown';
    if (githubLicenses.length === 0)
        return 'only-local';
    if (localLicenses.length === 0)
        return 'only-github';
    if (localLicenses.length > 1 || githubLicenses.length > 1)
        return 'ambiguous';
 ...
 */
function relateRefs(local, github) {
    const mk = (relation, relationReason) => ({
        local,
        github,
        relation,
        relationReason,
 ...
 */
export async function crossCheckWithGithub(sbom, options = {}) {
    ...
    if (seen.length > 1) {
        notes.push(`this checkout has ${seen.length} remotes (${seen.map((r) => r.name).join(', ')}); only ${chosen.name} was compared, ` +
    ...
    if (host !== 'github.com' && host !== 'www.github.com') {
        return noComparison('remote-not-github', `the ${chosen.name} remote points at ${parsed.host}, and this cross-check only speaks to github.com; ` +
            'an ssh config alias for github.com looks exactly like this from here and cannot be told apart without resolving it', sbom, unknownRefs, repository, notes);
    }
    const got = await fetchSbom(repository);
    if (!got.ok)
        return noComparison(got.status, got.reason, sbom, unknownRefs, repository, notes);
    ...
    if (doc.ref.repository !== null && doc.ref.repository.toLowerCase() !== `${repository.owner}/${repository.name}`.toLowerCase()) {
        notes.push(`the document GitHub returned describes ${doc.ref.repository}, not the ${repository.owner}/${repository.name} that was asked for; ` +
            'a renamed or redirected repository lands here, and everything below is a comparison against what was returned');
    }
    notes.push("GitHub builds this graph from the repository's default branch and rebuilds it on push, so it can lag; " +
    ...
    const sideOfLocal = (es) => ({
        versions: distinctBy(es.filter((e) => e.version !== null).map((e) => ({ value: e.version, kind: e.versionKind })), (v) => `${v.value} ${v.kind}`),
        copies: es.length,
        licenses: [...new Set(es.map((e) => e.license).filter((x) => typeof x === 'string' && x !== ''))].sort(),
    });
    const sideOfGithub = (ps) => ({
        versions: distinctBy(ps.filter((p) => p.version !== null && p.versionKind !== null).map((p) => ({ value: p.version, kind: p.versionKind })), (v) => `${v.value} ${v.kind}`),
        copies: ps.length,
        licenses: [...new Set(ps.map((p) => p.license).filter((x) => typeof x === 'string' && x !== ''))].sort(),
    });
    ...
    for (const c of byKey.values()) {
        if (c.presence !== 'both' || c.local === null || c.github === null)
            continue;
    ...
    const licenseDisagreements = packages
    ...
    if (licenseDisagreements.length > 0) {
        notes.push('licence values are compared as exact strings, and neither side is authoritative: ' +
    ...
    if (sbom.status !== 'ok') {
        notes.push(`the local reading returned status ${sbom.status} (${sbom.reason ?? 'no reason recorded'}), ` +
        ...
        notes.push(`the local side is ${sbom.transitiveClosure === 'absent' ? 'a direct-dependency list, not a closure' : 'only partly a closure'}, ` +
