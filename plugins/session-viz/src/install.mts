...
export interface Target {
  ...
  id: string
  label: string
  /** Where this harness reads skills from. */
  skills: string
  /** True when the harness is present on this machine at all. */
  present: boolean
  /** How this harness decides to run a skill, when it differs from Claude Code. */
  note?: string
}
...
export function targets(): Target[] {
  const home = homedir()
  const dir = (...p: string[]): string => join(home, ...p)
  const has = (d: string): boolean => { try { return statSync(dirname(d)).isDirectory() } catch { return false } }
...
const stamp = (root: string): string => `${MARK} from ${root} -->`
 ...
 */
export function install(target: Target, { dryRun = false }: { dryRun?: boolean } = {}): Written[] {
...
export interface Installed {
  skill: string
  from: string
  /** False when the copy points at a plugin root that is no longer there, or
   *  at a different checkout than the one running now. */
  current: boolean
}

/** What this tool has installed into a target, and whether it still resolves. */
export function installed(target: Target): Installed[] {
  const root = pluginRoot()
  const out: Installed[] = []
  let names: string[] = []
  try {
  ...
  for (const name of names) {
    let body: string
    try { body = readFileSync(join(target.skills, name, 'SKILL.md'), 'utf8') } catch { continue }
...
does it better, and two copies of ten skills would compete for ten commands.`)
  ...
  if (flag('--list')) {
    ...
    if (flag('--json')) { await emitJson(model); process.exit(0) }
  ...
  if (!chosen.length) {
    ...
    for (const t of all) console.error(`  ${t.label.padEnd(8)} ${t.skills}`)
