...
export function readConfig(): PluginConfig | null {
  try {
    return JSON.parse(readFileSync(CONFIG, 'utf8')) as PluginConfig
  } catch {
    return null
  }
}
...
const PAGE = (nonce: string, defaultUrl: string): string => `<!doctype html>
...
async function run(): Promise<void> {
  ...
  if (argv.includes('--show')) {
    const cfg = readConfig()
    if (!cfg) { console.log('no config at ' + CONFIG); return }
  ...
  if (argv.includes('--forget')) {
    if (existsSync(CONFIG)) { rmSync(CONFIG); console.log(`removed ${CONFIG}`) }
