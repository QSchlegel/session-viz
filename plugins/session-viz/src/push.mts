...
/**
 * Every key path that may appear on the wire, and what it is.
 *
 * This table is the single source for three things that must never disagree:
 * the disclosure a person reads, the digest their consent is bound to, and the
 * runtime check that refuses an undisclosed field. Generating all three from
 * one list is the only reason they cannot drift apart — the alternative is a
 * prose disclosure maintained by hand beside a payload maintained by hand,
 * which is how a field ends up leaving that nobody ever named.
 */
export interface FieldSpec {
  /** Dotted key path as it appears in the JSON body. */
  path: string
  says: string
}
...
export const FIELDS: readonly FieldSpec[] = [
  {
    path: 'document.html',
    says:
      'THE WHOLE REPORT, VERBATIM. The exact bytes of the HTML file /qpact just wrote and ' +
  ...
  { path: 'sessionId', says: "The harness's id for this session. The console keys on it, so re-running /qpact corrects the report rather than adding a second one." },
  {
    path: 'repo',
    says:
      'The LAST SEGMENT of the working directory — the repository directory name. The route ' +
  ...
  },
  {
    path: 'branch',
    says:
      'The git branch name, verbatim and unrewritten. Teams routinely name branches after the ' +
...
export const HEADERS: readonly FieldSpec[] = [
...
export interface ReportPayload {
  ...
  document: { mediaType: string; bytes: number; sha256: string; html: string }
...
export function repoLabel(cwd: string | null | undefined): string | null {
  ...
  if (base === basename(homedir())) return null
  return base
}
...
export function reportPayload(args: {
  ...
  if (!sessionId) throw new Error('this spine has no sessionId — nothing to key a report on')
...
export function keyPaths(value: unknown, prefix = ''): string[] {
  if (Array.isArray(value)) {
    const at = `${prefix}[]`
    const out = new Set<string>([at])
    for (const v of value) for (const p of keyPaths(v, at)) out.add(p)
    return [...out]
  }
 ...
 */
export function undisclosed(payload: unknown): { extra: string[]; missing: string[] } {
 ...
 * wrong.
 */
export const REDACTION_POSTURE: readonly string[] = [
 ...
 */
export function standingDisclosure(): string[] {
  const L: string[] = []
  const pad = (s: string): string => s.padEnd(22)
  const wrap = (text: string, indent: string): string[] => {
    const words = text.split(' ')
    const lines: string[] = []
    let cur = ''
    for (const w of words) {
      if (cur && (cur + ' ' + w).length > 74) { lines.push(cur); cur = '' }
      cur = cur ? `${cur} ${w}` : w
    }
...
export const disclosureDigest = (lines: string[] = standingDisclosure()): string => sha256(lines.join('\n'))
...
const wrapAt = (text: string, width: number): string[] => {
  const out: string[] = []
  let cur = ''
  for (const w of text.split(' ')) {
    if (cur && (cur + ' ' + w).length > width) { out.push(cur); cur = '' }
    cur = cur ? `${cur} ${w}` : w
  }
  if (cur) out.push(cur)
  return out
 ...
 */
export function documentCounts(html: string): { homePaths: number; emails: number } {
  const home = homedir()
...
export interface PushState {
   ...
   *  current text no longer digests to this. */
  disclosure?: { version: string; sha256: string }
  ...
  skipped?: string[]
  /**
   * Withhold the rendered page, keep sending the bounded facts.
   *
   * The two payloads expose differently and this is the switch that says so.
   * The document is every prompt in the session, verbatim; the index tier is
   * counts, bands and names, and it is what makes a workspace roll-up complete.
   * Somebody who wants the second without the first should not have to choose
   * between shipping their prompts and vanishing from their team's numbers.
   */
...
export const pushPaths = (): string[] => configDirs().map((d) => join(d, STATE_FILE))
 ...
 */
function savePush(state: PushState): string {
  ...
  throw new Error(
    `cannot record your choice — permission denied at ${refused.join(', ')}. ` +
...
export function withholdDocument(on: boolean): { path: string; lines: string[] } {
  const prev = loadPush()
...
export interface TurnOnResult {
  on: boolean
  /** Where consent was recorded, when it was. */
  path?: string
  destination?: string
  lines: string[]
}
...
export interface ShipResult {
  ...
  url?: string
  bytes?: number
  /** One line a person can act on. Empty only when shipped. */
  reason: string
  /** What the command should print, in order. */
  lines: string[]
}
 ...
 */
export async function shipReport(args: ShipArgs): Promise<ShipResult> {
  const lines: string[] = []
  const fail = (reason: string, extra: string[] = []): ShipResult => ({
  ...
  try {
    ...
    return fail(`could not read the report at ${args.reportPath}: ${(e as Error).message}`)
  }
  ...
  if (drift.extra.length || drift.missing.length)
    return fail(
      `refusing to send a payload the disclosure does not describe` +
...
if (isMain) {
  ...
  const say = (ls: string[]): void => { for (const l of ls) console.log(l) }
  ...
  try {
    ...
    if (on) {
      ...
      if (state.enabled) console.log(`  ${why}`)
