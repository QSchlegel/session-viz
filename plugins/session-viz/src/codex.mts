...
import type { ContentBlock, SessionFile, ToolInput, TranscriptRecord, UsageRecord } from './extract.mjs'
 ...
 */
export function isCodexTranscript(file: string): boolean {
  if (ROLLOUT.test(basename(file))) return true
  let fd = -1
  try {
    fd = openSync(file, 'r')
 ...
 */
export function codexProject(file: string, root?: string | null): string {
 ...
 */
export function listCodexSessions(root: string, projectFilter?: string | null): SessionFile[] {
  ...
  const walk = (dir: string, depth: number): void => {
    ...
    for (const e of entries) {
      ...
      if (!ROLLOUT.test(e.name)) continue
...
/** Cumulative token counters, as Codex reports them. `cached` is a subset of
 *  `input` and `reasoning` a subset of `output` — adding them double counts. */
...
interface CodexPayload {
  ...
  arguments?: unknown
  input?: unknown
  invocation?: { server?: unknown; tool?: unknown } | null
}
...
/** A normalised tool call, in the vocabulary extract.mts harvests. */
interface Block {
  name: string
  input: ToolInput
}
...
const INJECTED_PROMPT =
  /^\s*(<(environment_context|user_instructions|turn_aborted|image|app-context|collaboration_mode|permissions|recommended_plugins|multi_agent_mode|automation_context)\b|#+\s*AGENTS\.md instructions|Warning: apply_patch was requested)/
 ...
 */
const keep = (v: unknown, prev: number): number =>
  typeof v === 'number' && Number.isFinite(v) && v >= 0 ? v : prev
 ...
 */
function patchBlocks(patch: string): Block[] {
  const out: Block[] = []
  let op = ''
  let path = ''
  let added: string[] = []
  ...
  for (const line of patch.split('\n')) {
    ...
    if (m) {
      flush()
      if (out.length >= MAX_PATCH_FILES) return out
      op = m[1]!
      path = m[2]!.trim()
      continue
    }
    ...
    if (path && line.startsWith('+')) added.push(line.slice(1))
  }
  flush()
 ...
 * near-miss.
 */
function functionCallBlocks(name: string, argsRaw: unknown): Block[] {
  ...
  try {
    ...
    const parsed: unknown = JSON.parse(str(argsRaw) || '{}')
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) a = parsed as Record<string, unknown>
  } catch {
    a = {}
  }
  switch (name) {
...
function customCallBlocks(name: string, input: unknown): Block[] {
  const body = str(input)
  if (name === 'apply_patch') return patchBlocks(body)
 ...
 */
export async function* codexRecords(file: string): AsyncGenerator<TranscriptRecord> {
  let seq = 0
  ...
  let effort = ''
  let lastTs = ''
  let cum: [number, number, number, number] | null = null
  ...
  const flush = (): TranscriptRecord[] => {
    const held = buf
    buf = []
    return held
  }

  const mk = (ts: string, type: string, extra: Partial<TranscriptRecord> = {}): TranscriptRecord => ({
    type,
    uuid: `${sid}:${seq++}`,
    timestamp: ts,
    ...extra,
  ...
  const neutral = (ts: string, extra: Partial<TranscriptRecord> = {}): TranscriptRecord =>
    mk(ts, 'codex-event', extra)

  const assistant = (ts: string, content: ContentBlock[]): TranscriptRecord =>
    mk(ts, 'assistant', { message: { model: model || undefined, content }, effort: effort || undefined })

  const textBlock = (text: string): ContentBlock => ({ type: 'text', text })
  const toolBlock = (b: Block): ContentBlock =>
    ({ type: 'tool_use', name: b.name, input: b.input }) as unknown as ContentBlock
   ...
   */
  const usageDelta = (info: CodexPayload['info']): UsageRecord | null => {
    ...
    if (!din && !dcache && !dwrite && !dout) return null
    return {
  ...
  for await (const line of readLines(file)) {
    ...
    try {
      ...
      if (!ts) continue
      lastTs = ts
      const p: CodexPayload = raw.payload && typeof raw.payload === 'object' ? raw.payload : {}
      ...
      if (kind === 'session_meta') {
        ...
        if (str(p.id)) sid = str(p.id)
        const src = p.source
        noHuman =
          str(p.thread_source) === 'subagent' ||
          !!str(p.parent_thread_id) ||
          (!!src && typeof src === 'object' && 'subagent' in (src as object)) ||
        ...
        if (review && !sawHuman) noHuman = true
        ...
        // surfaced this.
        if (m && !review) model = m
        if (str(p.effort)) effort = str(p.effort)
        ...
        if (IDE_CONTEXT.test(text)) {
          ...
          if (m && m.index !== undefined) text = text.slice(m.index + m[0].length)
        }
        text = text.trim()
        const hasImage =
        ...
        if (role === 'assistant' && text && text !== lastAgentText) {
          ...
          if (role === 'assistant') lastAgentText = null
          out.push(neutral(ts))
        }
      } else if (kind === 'response_item' && (pt === 'function_call' || pt === 'custom_tool_call')) {
        const name = str(p.name) || pt
        const blocks =
          pt === 'function_call' ? functionCallBlocks(name, p.arguments) : customCallBlocks(name, p.input)
        ...
        if (kind === 'event_msg' && pt === 'task_complete') taskOpen = false
        out.push(neutral(ts))
      }
    } catch {
    ...
    for (const rec of out) {
      if (rec.type === 'assistant') {
        for (const held of flush()) yield held
        buf.push(rec)
        ...
        if (buf.length > MAX_HELD) for (const held of flush()) yield held
      } else {
        yield rec
      }
    }
    if (usage) for (const held of flush()) yield held
  }
  ...
  if (!sawMeta) yield mk(lastTs, 'session_meta', { sessionId: sid })
}
