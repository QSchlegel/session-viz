...
export async function* codexRecords(file: string): AsyncGenerator<TranscriptRecord> {
  ...
  for await (const line of rl) {
    ...
    try {
      ...
      if (!ts) continue // nothing can be attributed to a record with no clock
      lastTs = ts
      ...
      if (kind === 'session_meta') {
        ...
        if (str(p.model)) model = str(p.model)
        if (str(p.effort)) effort = str(p.effort)
        if (str(p.model) === 'codex-auto-review') noHuman = true
    ...
    if (usage) while (buf.length) yield buf.shift()!
  }
  ...
  if (!sawMeta && lastTs) yield mk(lastTs, 'session_meta', { sessionId: sid })
}
