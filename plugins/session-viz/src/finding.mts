...
export function taskClass(run: Run): string {
  ...
  if (run.kind === 'subagent') return run.family ? contribSlug(run.family) : 'other'
  if (run.task) return withoutRepo(contribSlug(run.task), run.repo)
 ...
 * actually about.
 */
export function withoutRepo(slug: string, repo: string | null | undefined): string {
  if (!repo) return slug
  ...
  const flat = (s: string): string => s.replace(/_/g, '-')
  const r = flat(contribSlug(repo))
  if (!r || r === 'unknown') return slug
  ...
  if (cur === flat(slug)) return slug        // nothing matched; leave it untouched
