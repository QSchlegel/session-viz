...
import type { SavedTo } from './home.mjs'
...
import type { ContribFinding } from './finding.mjs'
import type { Run } from './runs.mjs'
...
interface RunsJson { runs?: Run[] }

async function collect(since: string | null): Promise<Run[]> {
  ...
  return (JSON.parse(stdout) as RunsJson).runs || []
}

// ---------------------------------------------------------------- the ledger of what was sent

interface SentEntry { at: string; week: string }
interface State {
  schema_version: string
  sent: Record<string, SentEntry>
}

const emptyState = (): State => ({ schema_version: 'contrib_v1', sent: {} })
 ...
 * canonical one.
 */
function runKey(r: Run): string {
  let file = r.file
  try { file = realpathSync(r.file) } catch { /* not a path on disk: the string is the identity */ }
...
interface Selection {
  all: Run[]
  eligible: Run[]
  findings: ContribFinding[]
  alreadySent: number
  warm: number
  /** Findings this machine's own copy of the server's rules refused to send. */
  held: { run: Run; finding: ContribFinding; err: string }[]
}

function select(runs: Run[], state: State, force: boolean): Selection {
  const now = Date.now()
  const eligible: Run[] = []
  const findings: ContribFinding[] = []
  const held: Selection['held'] = []
  let alreadySent = 0, warm = 0

  for (const r of runs) {
    if (!force && state.sent[runKey(r)]) { alreadySent++; continue }
    const touched = Date.parse(r.ended || r.started)
    if (Number.isFinite(touched) && now - touched < WARM_MS) { warm++; continue }
    const f = toFinding(r)
    ...
    if (err) { held.push({ run: r, finding: f, err }); continue }
    eligible.push(r)
    findings.push(f)
  }
...
function disclose(sel: Selection): string[] {
  ...
  if (weeks.length) out.push(`  weeks ${weeks[0]} … ${weeks[weeks.length - 1]}`)
  out.push('')
  ...
  if (w) {
    ...
    const distinct = (f: (r: Run) => string): string =>
      [...new Set(sel.eligible.map(f).filter(Boolean))].slice(0, 4).join(', ') || '(none recorded)'
...
interface ContribReply {
  accepted: number
  rejected: { i: number; err: string }[]
}
 ...
 */
async function postBatch(cfg: Config, findings: ContribFinding[]): Promise<ContribReply> {
  ...
  {
    const body = (await r.json().catch(() => ({}))) as Partial<ContribReply> & { error?: string }
    if (r.status >= 500) {
      throw new Error(
        `HTTP ${r.status} — ${findings.length} finding(s) of unknown fate, for the same\n` +
    ...
    if (!r.ok && !Array.isArray(body.rejected)) throw new Error(body.error || `HTTP ${r.status}`)
...
if (isMain) {
  ...
  try {
    const state = loadState<State>() || emptyState()
    ...
    if (argv.includes('--review')) {
      for (const line of disclose(sel)) console.log(line)
