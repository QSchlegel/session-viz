...
interface Turn {
  ...
  score: TurnScore
  steering?: unknown
  /** The files this turn's tool calls named, most-touched first. Absent on a
   *  reading extracted with --no-paths and on any spine written before the
   *  field existed -- `session.recordedPaths` is what tells those two apart,
   *  and an empty list here means nothing on its own. */
  files?: TurnFileTouch[]
}

/** One file a turn named, and how many of its tool calls named it. */
interface TurnFileTouch {
  path: string
  count: number
}
...
interface Session {
   ...
   *  Three values and not two: `false` is a refusal (--no-paths) and
   *  `undefined` is a spine that predates the field, and the appendix has to
...
interface IntentItem {
  ...
  turns?: number[]
  /** The intent store's canonical copy of the same citation. `turns` above is
   *  a MIRROR the store maintains only because this file reads it there; the
   *  copy inside provenance is the one that survives being flattened into
   *  another list. Read as a fallback so the citation does not go silent if the
   *  mirror is ever dropped -- and a silent citation is not a visible failure
   *  here, it is a thread that quietly stops naming its own turns. */
  provenance?: { turns?: number[] }
}
 ...
 */
const citedTurns = (i: IntentItem | null | undefined): number[] =>
  (i?.turns ?? i?.provenance?.turns ?? []).filter((n) => Number.isInteger(n))
...
/** One earlier session's conclusions, as the intent store emits them under
 *  `prior`. Read here so the page can DRAW them; nothing in this file may
 *  present one as work this session did. */
interface PriorIntent {
  sessionId?: string
  sessionsAgo?: number
  daysAgo?: number | null
  intents?: IntentItem[]
...
interface Intent {
  ...
  sessionId?: string
  graph?: IntentGraph
  /** Earlier sessions of this project, newest first. Absent when the run had no
   *  store, which is not the same as a project with no history. */
  prior?: PriorIntent[]
  /** The store's own statement about age. `note` is quoted VERBATIM wherever
   *  carried material is drawn: it is written once, in intent.mts, precisely so
   *  a renderer does not paraphrase it into a confidence claim. */
  staleness?: { note?: string; sessionsHeld?: number; priorOmitted?: number; oldestRecordedAt?: string | null }
}
...
const unfinished = (i: IntentItem): boolean => UNFINISHED.has(String(i.status || ''))
...
function statusPill(status: unknown): string {
  ...
  return `<span class="pill${glyph ? ` st-${kindClass(s)}` : ''}">${
    glyph ? `<svg class="sg" viewBox="0 0 12 12" aria-hidden="true" focusable="false">${glyph}</svg>` : ''
  }${esc(s)}</span>`
}
...
const threadSlug = (title: string): string =>
  String(title).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 48)

/**
 * What the spine can say about file paths, in the same four states bundle.mjs
 * derives its sentence from -- and for the same reason. `false` is a refusal
 * and `undefined` is a reading older than the field, and both of them render an
 * empty table unless something tells them apart. An empty table under a task
 * reads as "this task changed nothing", which is a claim about the work; the
 * truth is a claim about the reading.
 */
type PathState = 'recorded' | 'refused' | 'unknown-empty' | 'unknown-present'

const pathsPresent = (session: Session): number =>
  session.turns.reduce((n, t) => n + (t.files?.length ?? 0), 0)

const pathState = (session: Session): PathState => {
  if (session.recordedPaths === true) return 'recorded'
  if (session.recordedPaths === false) return 'refused'
...
/** One file attributed to one task, with the thing that makes the attribution
 *  honest riding alongside it. */
interface TaskFile {
  path: string
  /** Tool calls, from the cited turns only, that named this path. */
  count: number
  /** Titles of the OTHER tasks whose cited turns also named it. Non-empty means
   *  the file belongs to none of them exclusively. */
  alsoIn: string[]
}

interface TaskAttribution {
  files: TaskFile[]
  /** Turns the analysis cited that the spine holds no record of. */
  missing: number[]
  /** Cited turns whose record carries no file list at all. */
  quiet: number
  /** Cited turns, as the analysis wrote them, filtered to integers. */
  cited: number[]
}
 ...
 */
function attribute(session: Session, items: IntentItem[]): TaskAttribution[] {
  const byIndex = new Map(session.turns.map((t) => [t.index, t]))
  const raw = items.map((it): { cited: number[]; counts: Map<string, number>; missing: number[]; quiet: number } => {
    ...
    for (const n of cited) {
      ...
      for (const f of files) counts.set(f.path, (counts.get(f.path) || 0) + (f.count || 0))
    }
  ...
  raw.forEach((r, i) => {
    for (const path of r.counts.keys()) owners.set(path, [...(owners.get(path) || []), i])
  })
...
function unclaimed(session: Session, attr: TaskAttribution[]): { files: number; touches: number } {
  const claimed = new Set(attr.flatMap((a) => a.files.map((f) => f.path)))
  const all = new Map<string, number>()
  for (const t of session.turns) for (const f of t.files || []) all.set(f.path, (all.get(f.path) || 0) + (f.count || 0))
  let files = 0
  let touches = 0
...
const wrap = (s: string, indent = '  ', width = 76): string => {
  const out: string[] = []
  let line = ''
...
function followupPrompt(
  ...
  if (other) {
    ...
    L.push(
      wrap(
        `The conclusion below was written about session ${other.slice(0, 8)}. The transcript it was ` +
    ...
    L.push('')
  } else L.push(`Follow-up on one unfinished thread from session ${sid.slice(0, 8)}, as /qpact read it.`)
  L.push('')
  L.push('THREAD')
  ...
  if (other)
    L.push(
      `  Turns cited: ${
        a.cited.length ? `${a.cited.join(', ')} — of session ${other.slice(0, 8)}, not of the spine this was rendered against.` : 'none.'
      }`
    )
  else {
    L.push(
  ...
  if (other)
    ...
    L.push(
      wrap(
        `The turn numbers cited here belong to session ${other.slice(0, 8)}, and the spine holds ` +
    ...
    L.push(
      wrap(
        'This reading was extracted with --no-paths, so the spine records that a file was ' +
    ...
    L.push(
      wrap(
        'This spine predates the field that records which files a turn touched, so nothing ' +
    ...
    L.push(
      wrap(
        a.cited.length - a.missing.length === 0
          ...
          : `None. ${plural(a.cited.length - a.missing.length, 'cited turn')} ${
              a.cited.length - a.missing.length === 1 ? 'is' : 'are'
            } in the spine and not one of them made a tool call naming a file. A turn that did its ` +
    ...
    for (const f of shown)
      L.push(
        `  ${f.path.padEnd(w)}  ${String(f.count).padStart(4)} tool ${f.count === 1 ? 'call' : 'calls'}` +
          (f.alsoIn.length ? `   also under: ${f.alsoIn.join('; ')}` : '')
      )
    if (rest.length)
      L.push(
        `  ...and ${plural(rest.length, 'more file')}, not listed here, between them named by ` +
          `${plural(rest.reduce((n, f) => n + f.count, 0), 'tool call')}.`
      )
  }
  if (!other && a.quiet) L.push(`  (${plural(a.quiet, 'cited turn')} named no file at all.)`)
  L.push('')
  ...
  if (!other && (state === 'recorded' || state === 'unknown-present')) {
    ...
    L.push(
      wrap(
        'A file is on it because a turn cited for this thread made a tool call naming that ' +
    ...
    if (state === 'unknown-present')
      L.push(
        wrap(
          'This spine also predates the flag that records whether paths were kept, so the ' +
  ...
  L.push(
    wrap(
      `Nothing states what remains to be done. "${status}" is the status the analysis assigned ` +
...
function renderIntents(session: Session, intent: Intent | null | undefined): string {
...
function renderAppendix(session: Session, intent: Intent | null | undefined): string {
  const items = intent?.intents ?? []
  const state = pathState(session)
  ...
  if (!items.length)
    return `${head}
<div class="card"><p style="margin:0">The analysis named no tasks, so there is nothing to attribute files to. ${
      state === 'recorded'
        ...
        : 'Whether the spine records which files were touched is a separate question, answered in the evidence package below.'
    }</p></div>`

  if (state !== 'recorded' && state !== 'unknown-present')
    return `${head}
<div class="card apx-none">
  <p style="margin:0 0 10px"><strong>No file can be attributed to any task in this reading.</strong> ${
    items.length === 1 ? 'The one task below is' : `All ${items.length} tasks below are`
  } still here, with the turns cited for ${items.length === 1 ? 'it' : 'them'} — what is missing is the other half of the join, not the tasks.</p>
  ...
  <ul class="apx-tasks">${items
    .map(
      (i) =>
        `<li><b>${esc(i.title)}</b> ${statusPill(i.status)} — ${
          citedTurns(i).length ? `turns ${esc(citedTurns(i).join(', '))}` : 'no turns cited'
        }</li>`
    )
    .join('')}</ul>
</div>`
      ...
      if (!a.cited.length)
        ...
        body =
          a.cited.length - a.missing.length === 0
            ...
            : `<p class="apx-say">${plural(a.cited.length - a.missing.length, 'cited turn')} ${
                a.cited.length - a.missing.length === 1 ? 'is' : 'are'
              } in the spine, and none of them made a tool call naming a file. Work done through a shell command looks exactly like this: nothing reads paths out of command text.</p>`
      else {
        const rest = a.files.slice(MAX_FILES_LISTED)
  ...
  <tbody>${a.files
    ...
    .map(
      (f) =>
        `<tr${f.alsoIn.length ? ' class="apx-shared"' : ''}><td><code>${esc(f.path)}</code></td><td class="num">${f.count}</td><td>${
          f.alsoIn.length ? esc(f.alsoIn.join('; ')) : '<span class="dim">this task only, among the tasks named</span>'
        }</td></tr>`
    )
    .join('')}${
      rest.length
        ? `<tr><td class="dim">and ${plural(rest.length, 'more file')}, not listed</td><td class="num dim">${rest.reduce(
            (n, f) => n + f.count,
            0
          )}</td><td class="dim">the list is cut at ${MAX_FILES_LISTED}; the count beside it is theirs</td></tr>`
        : ''
    }</tbody>
</table></div>`
      }
...
<div class="card apx-how">
  ...
  <p style="margin:10px 0 0">A file named only inside a shell command is not here at all, so a task that worked through <code>sed</code> or <code>git</code> can show few files or none. ${
    sharedRows
      ...
      : 'No file below is attributed to more than one task in this reading, which is a fact about these particular citations and not a property of the method.'
  }</p>
  ${
  ...
  <p style="margin:10px 0 0" class="dim">${
    rest.files
      ...
      : 'Every file this session touched is claimed by at least one task above.'
  }</p>
</div>
${tables}`
}
...
function renderGraph(session: Session, intent: Intent | null | undefined, bom?: RenderMeta['bom']): string {
  ...
  const cited = (list: IntentItem[] | undefined) => (list ?? []).map((i) => ({ ...i, turns: citedTurns(i) }))
  ...
  const loose = layout.isolated
    ? `<span class="ghalf dim" title="${esc(
        cappedEdges
          ...
          : 'They carry no edge at all, so the grid is an arrangement and not a measurement.'
      )}">${layout.isolated} connect to nothing drawn</span>`
    : ''
...
<div class="gfoot">
  ...
  <p class="gcarry">${
    carriedNodes.length
      ...
      : 'Nothing from an earlier session is drawn on this canvas.'
  }${
    (intent?.staleness?.priorOmitted ?? 0) > 0
  ...
  <p class="gcarry dim">${esc(
    intent?.staleness?.note ||
      'Nothing has re-checked whether a conclusion from an earlier session still holds. Read the age as age, not as confidence.'
  )}</p>`
      : ''
  }
...
// ---------------------------------------------------------------- the package
//
// The page answers the question while the tab is open. Someone eventually has
// to answer it on a ticket, months later, to a client or a risk function, and
// that answer cannot be a browser tab. src/bundle.mts turns the same spine into
// a zip; this wires it to a button and prints what the record cannot show
// BEFORE anyone attaches it to anything.

/** The spine as the packager's own signature describes it: extract.mts's
 *  `Session`, reached through `buildBundle` so this file keeps one `Session`
 *  name and cannot end up importing a second one that drifts from it. */
type SpineSession = Parameters<typeof buildBundle>[0]
type SpineTurn = SpineSession['turns'][number]
 ...
 */
function evidenceTurn(turn: Turn): SpineTurn {
  return {
    promptId: null,
    uuid: '',
    startedAt: '',
 ...
 * measurement.
 */
function evidenceSpine(session: Session): SpineSession {
...
if (isMain) {
  ...
  if (wantList || asked) {
    ...
    if (intent?.sessionId && session.sessionId && intent.sessionId !== session.sessionId)
      console.error(
        `warning: intent.sessionId (${String(intent.sessionId).slice(0, 8)}) does not match the spine ` +
    ...
    if (wantList) {
      console.log(
        `${eligible.length} of ${items.length} thread(s) were left abandoned or ongoing.\n` +
    ...
    if (!unfinished(items[pick]!)) {
      ...
      console.error(
        `"${items[pick]!.title}" is ${items[pick]!.status || 'unstated'}, so it has no drill-down. ` +
