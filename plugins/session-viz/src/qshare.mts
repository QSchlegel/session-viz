...
/**
 * The parts of the corpus this command reads. Structural and hand-written,
 * because corpus.mjs is invoked as a subprocess and its types do not cross that
 * boundary — which is exactly how this went wrong: it used to declare an
 * `exemplars.best`, the producer has only ever emitted `repeats`, `corrections`
 * and `worst`, and the compiler dutifully type-checked reads of a field that
 * was never there. Every entry here is present in the real output; nothing is
 * declared hopefully.
 */
interface CorpusModel {
  projects: CorpusProject[]
  /** Every session's digest, all of them. The only complete list in the model —
   *  `exemplars` and `incidents` are both selections. */
  sessions?: Array<Record<string, unknown>>
  exemplars?: { worst?: Array<Record<string, unknown>> }
...
function sessionPayload(m: CorpusModel, id: string): unknown {
  ...
  if (hits.length > 1) {
    ...
    throw new Error(
      `${hits.length} sessions start with ${id} — name one exactly:\n${lines.join('\n')}` +
...
interface PickRow {
  ...
  bytes: number
  textFields: number
  /** Fields in this row's payload that name a file, or null when the payload
   *  would not build and nothing was measured. Null is not zero: the page says
   *  "not measured" for it rather than reporting a reassuring number it does not
   *  have. */
  filePaths: number | null
  ambiguous: boolean
...
async function pickRows(m: CorpusModel): Promise<PickRow[]> {
  ...
  for (const p of m.projects) {
    ...
    let filePaths: number | null = null
    try {
      ...
      bytes = d.bytes; textFields = d.textFields; filePaths = d.filePaths
...
export function pickerPage(rows: PickRow[], nonce: string, shared: Set<string>): string {
  ...
  const filePathCount = measured.reduce((a, r) => a + (r.filePaths as number), 0)
...
${rows.map((r, i) => `<tr>
<td><input type="checkbox" id="c${i}" data-ref="${esc(r.ref)}" data-text="${r.textFields}"${
  shared.has(r.ref) ? ' data-shared="1"' : ''}></td>
<td><label for="c${i}"><span class="nm">${esc(r.name)}</span>${
  shared.has(r.ref) ? '<span class="done">shared</span>' : ''}${
  r.ambiguous ? '<span class="amb">two checkouts</span>' : ''}</label>
...
async function runPicker(cfg: Config): Promise<void> {
  ...
  const shared = new Set<string>(
    ((existing.shares as Array<{ ref?: string; kind?: string }>) || [])
  ...
  await new Promise<void>((resolve) => {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      const emit = (o: unknown): void => { if (!gone) res.write(JSON.stringify(o) + '\n') }
