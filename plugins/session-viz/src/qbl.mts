...
/**
 * What a backlog entry has to carry to survive the conversation that produced
 * it.
 *
 * The failure this shape exists to prevent is the note that reads "fix the
 * picker" three weeks later, when nobody remembers which picker, in which repo,
 * or what was wrong with it. So: `text` is what, `where` is where, and `why` is
 * why it matters — and `why` is nullable rather than invented, because a
 * plausible reason generated on the user's behalf is worse than a stated
 * absence.
 */
export interface Where {
  repo: string
  /** Absolute repository root at push time. Local state only; never sent. */
  root: string
  branch: string | null
  worktree: string | null
  /** Path within the repo the push happened in, when it was not the root. */
  path: string | null
}
...
export interface Item {
  id: string
  text: string
  /** Why it matters, as the person stated it. Null when they did not. */
  why: string | null
  where: Where
  tags: string[]
  /** Ids of items that must be closed before this one can start. */
  blockedBy: string[]
  state: 'open' | 'done'
  createdAt: string
  updatedAt: string
  closedAt?: string
  /** Where this row came from, so a mixed list can never blur the two. */
  origin: 'local' | 'shared'
  /** Shared only: who holds it, when somebody has accepted or been offered it. */
  heldBy?: string | null
  /** Shared only: the server's own state, which is richer than open/done. */
  sharedState?: string
}

interface Backlog {
  version: 1
  /** The project this file belongs to, written so a stray copy is identifiable. */
  project: string
  items: Item[]
}
 ...
 */
const slug = (s: string): string => s.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || 'project'
 ...
 */
export function projectKey(cwd: string): { root: string; checkout: string; name: string; key: string } {
...
const backlogPaths = (key: string): string[] => configDirs().map((d) => join(d, 'backlog', `${key}.json`))

const backlogTarget = (key: string): string => join(dirname(configTarget()), 'backlog', `${key}.json`)
 ...
 */
export function loadBacklog(key: string, project: string): Backlog {
  const here = backlogTarget(key)
  const p = existsSync(here) ? here : backlogPaths(key).find((q) => existsSync(q))
  const empty: Backlog = { version: 1, project, items: [] }
  if (!p) return empty
  try {
    const parsed = JSON.parse(readFileSync(p, 'utf8')) as Backlog
...
export function saveBacklog(key: string, b: Backlog): string {
  ...
  throw new Error(
    `cannot write the backlog — permission denied at ${refused.join(', ')}. ` +
 ...
 */
export function strayBacklogs(key: string, root: string): Array<{ path: string; items: Item[] }> {
  const out: Array<{ path: string; items: Item[] }> = []
  const seen = new Set<string>()
  const resolves = new Map<string, string>()
  const resolve = (r: string): string => {
  ...
  for (const dir of configDirs().map((d) => join(d, 'backlog'))) {
    ...
    for (const n of names) {
      ...
      try { doc = JSON.parse(readFileSync(path, 'utf8')) as Backlog } catch { continue }
      ...
      if (mine.length) out.push({ path, items: mine })
    }
  }
  return out
}
...
export interface Context {
  cwd: string
  /** The repository the backlog is keyed on. Worktrees of it collapse to here. */
  root: string
  /** The checkout the command was run in — the worktree, when it is one. */
  checkout: string
  name: string
  key: string
  branch: string | null
 ...
 */
export const PULL_PHRASES: readonly string[] = [
...
export function readsAsPull(arg: string): boolean {
...
export interface Ordering {
  next: Ranked[]
  held: Held[]
  /** Every criterion, in the order applied, and whether it changed anything. */
  basis: Array<{ criterion: string; separated: boolean; detail: string }>
  /** True when nothing but age distinguished one ready item from another. */
  ageAlone: boolean
  /** Blocker ids that name no item here. Reported, never silently satisfied. */
  danglingBlockers: Array<{ id: string; names: string }>
}

const ago = (fromIso: string, now: number): string => {
 ...
 */
export function order(items: Item[], ctx: { branch: string | null }, now: number): Ordering {
  const open = items.filter((i) => i.state === 'open')
  const byId = new Map(items.map((i) => [i.id, i]))
  const openIds = new Set(open.map((i) => i.id))

  const danglingBlockers: Ordering['danglingBlockers'] = []
  for (const i of open) {
    for (const b of i.blockedBy) {
      if (!byId.has(b)) danglingBlockers.push({ id: i.id, names: b })
    }
  }
  ...
  for (const i of open) {
    ...
    if (waiting.length) { held.push({ item: i, reason: `waiting on ${waiting.join(', ')}` }); continue }
    // Shared only: somebody else already holds it. Proposing it as "next"
    // would be proposing work that is not yours to take.
    if (i.heldBy) { held.push({ item: i, reason: `${i.sharedState || 'held'} by ${i.heldBy}` }); continue }
    ready.push(i)
  }

  const unblocksOf = (i: Item): string[] => open.filter((o) => o.blockedBy.includes(i.id)).map((o) => o.id)
  const onBranchOf = (i: Item): boolean => !!ctx.branch && i.where.branch === ctx.branch

  const ranked: Ranked[] = ready.map((item) => ({
  ...
  const distinct = <T,>(vs: T[]): boolean => new Set(vs).size > 1
  const unblockSeparated = distinct(ranked.map((r) => r.unblocks.length))
  const branchSeparated = distinct(ranked.map((r) => r.onBranch))

  for (const r of ranked) {
    if (r.unblocks.length) r.reasons.push(`unblocks ${r.unblocks.length} open item(s): ${r.unblocks.join(', ')}`)
    if (r.onBranch && branchSeparated) r.reasons.push(`filed on ${ctx.branch}, the branch checked out here`)
    r.reasons.push(`filed ${ago(r.item.createdAt, now)}`)
  }

  const basis: Ordering['basis'] = [
    {
...
export interface SharedResult {
  ok: boolean
  items: Item[]
  /** Why the shared backlog is not in this answer. Empty when it is. */
  reason: string
  url: string
}
 ...
 */
export function fromTask(t: TaskRow, me: string | undefined): Item {
 ...
 * before falling back.
 */
function netReason(e: unknown, timeoutMs: number): string {
  const err = e as Error & {
    code?: string
    cause?: { code?: string; message?: string; errors?: Array<{ code?: string }> }
  }
 ...
 */
export async function sharedItems(timeoutMs: number): Promise<SharedResult> {
  let cfg: Config
  try {
    cfg = config()
  } catch (e) {
    return { ok: false, items: [], url: '', reason: `${(e as Error).message}` }
  }
  ...
  try {
    ...
    const body = (await r.json().catch(() => ({}))) as { error?: string } | TaskRow[]
    if (!r.ok) {
    ...
    if (!Array.isArray(body)) return { ok: false, items: [], url, reason: `${url} answered something that is not a task list` }
 ...
 */
export async function pushShared(item: Item, timeoutMs: number): Promise<{ id: string } | { error: string }> {
  let cfg: Config
  try { cfg = config() } catch (e) { return { error: (e as Error).message } }
  const url = cfg.url.replace(/\/$/, '')
  const brief =
    `${item.text}\n\n` +
  ...
  try {
    ...
    if (!r.ok) return { error: `${url} refused the push: ${body.error || `HTTP ${r.status}`}` }
...
const mintId = (): string => 'b' + randomBytes(4).toString('hex')
 ...
 */
export function resolveId(items: Item[], given: string): { id: string } | { error: string } {
  const exact = items.find((i) => i.id === given)
  if (exact) return { id: exact.id }
  const hits = items.filter((i) => i.id.startsWith(given))
  if (hits.length === 1) return { id: hits[0]!.id }
  if (hits.length === 0) return { error: `no item here has an id starting with ${given}` }
  return { error: `${given} matches ${hits.length} items: ${hits.map((h) => h.id).join(', ')}` }
}

// ---------------------------------------------------------------- render

const rule = (n = 70): string => '─'.repeat(n)
...
interface PullView {
  ...
  limit: number
  /** Files whose items were folded in because they were keyed on a subdirectory. */
  adoptedFrom: string[]
}

function renderPull(v: PullView): string {
  const L: string[] = []
  ...
  if (v.adoptedFrom.length) {
    ...
    for (const p of v.adoptedFrom) L.push(`    ${p}`)
  }
  ...
  if (ordering.held.length) {
    ...
    L.push(rule())
    for (const h of ordering.held) L.push(`  ${h.item.id}  ${h.item.text.split('\n')[0]!.slice(0, 52)}  —  ${h.reason}`)
  }
  ...
  if (v.ordering.danglingBlockers.length) {
    ...
    for (const d of v.ordering.danglingBlockers) L.push(`    ${d.id} waits on ${d.names}, which is not here`)
  }
 ...
 */
function countFlag(argv: string[], name: string, fallback: number): number {
 ...
 */
function openBacklog(ctx: Context): { b: Backlog; adoptedFrom: string[] } {
  ...
  for (const stray of strayBacklogs(ctx.key, ctx.root)) {
    ...
    for (const i of fresh) { b.items.push(i); have.add(i.id) }
  ...
  if (adoptedFrom.length) { try { saveBacklog(ctx.key, b) } catch { /* still listed, still said */ } }
...
if (isMain) {
  ...
  const scope: 'local' | 'shared' = has('--shared') ? 'shared' : 'local'
  ...
  try {
    ...
    if (has('--phrases')) {
      await writeOut(
        'An argument equal to one of these — after lowercasing, dropping punctuation and\n' +
    ...
    if (closing) {
      if (scope === 'shared') {
        console.error(
          'Closing a shared task is a handoff transition, not a backlog edit: only\n' +
      ...
      if (item.state === 'done') { console.log(`${item.id} was already closed.`); process.exit(0) }
      item.state = 'done'
      ...
      if (!waiting.length) console.log('        nothing was waiting on it.')
      if (freed.length) console.log(`        ${freed.length} item(s) are no longer blocked: ${freed.map((f) => f.id).join(', ')}`)
      for (const s of stuck) {
        console.log(`        ${s.id} was waiting on it and is STILL held, by ${s.blockedBy.filter((x) => stillOpen.has(x)).join(', ')}`)
      }
      if (adoptedFrom.length) console.log(`\n  items were folded in from ${adoptedFrom.join(', ')}`)
      process.exit(0)
    }
    ...
    if (forcedPush && forcedPull) { console.error('--push and --pull say opposite things. Pick one.'); process.exit(1) }
    const pull = forcedPull || (!forcedPush && (!free || readsAsPull(free)))

    if (forcedPush && !free) { console.error('--push with nothing to push. Give the item as free text.'); process.exit(1) }
    ...
    if (!pull) {
      ...
      if (local && blockedBy.length) {
        for (let n = 0; n < blockedBy.length; n++) {
          ...
          if ('error' in r) { console.error(`--blocked-by ${blockedBy[n]}: ${r.error}`); process.exit(1) }
      ...
      if (asJson) { await emitJson({ pushed: item, path, adoptedFrom }); process.exit(0) }
    ...
    let effective: 'local' | 'shared' = scope
    let adoptedFrom: string[] = []
