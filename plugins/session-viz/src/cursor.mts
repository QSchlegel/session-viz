...
function gitRootOf(start: string): string | null {
  ...
  for (let i = 0; i < 40 && dir && dir !== '/' && dir !== home; i++) {
    if (existsSync(join(dir, '.git'))) {
      ...
      if (below !== dir && !repoTracks(dir, below)) return below
      return dir
    }
    below = dir
...
const tracksCache = new Map<string, boolean>()
 ...
 */
function repoTracks(repo: string, path: string): boolean {
  ...
  try {
    ...
    if (r.status === 0 && !r.error) out = (r.stdout || '').trim().length > 0
