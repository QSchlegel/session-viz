...
export function version(): string {
  try {
    ...
    const j = JSON.parse(readFileSync(manifest, 'utf8')) as { version?: string }
...
export const installed = (): boolean =>
  fileURLToPath(import.meta.url).includes(join('.claude', 'plugins', 'cache'))
 ...
 */
export function newerInstalled(): string | null {
  const mine = version()
  if (mine === 'unknown') return null
  ...
  try {
    for (const market of readdirSync(root, { withFileTypes: true })) {
      ...
      for (const v of versions) {
        if (cmp(v, mine) > 0 && (!best || cmp(v, best) > 0)) best = v
      }
    }
 ...
 */
export function versionNote(): { version: string; stale: string | null; line: string } {
  const v = version()
